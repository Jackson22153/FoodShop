...
public class RoleConverter implements Converter<Jwt, Collection<GrantedAuthority>>{
    ...
    public Collection<GrantedAuthority> convert(Jwt token) {
        Map<String, Object> roles = 
            (Map<String, Object>) token.getClaims().get(WebSecurityConfig.REALM_ACCESS_CLAIM);
        if(roles!=null){
            List<GrantedAuthority> userRoles = ((List<String>) roles.get(WebSecurityConfig.ROLES_CLAIM)).stream()
