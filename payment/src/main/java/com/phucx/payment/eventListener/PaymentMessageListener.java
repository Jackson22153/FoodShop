...
public class PaymentMessageListener {
    ...
    public String paymenthandler(String message){
        ...
        try {
            TypeReference<EventMessage<PaymentDTO>> typeReference = 
                new TypeReference<EventMessage<PaymentDTO>>() {};
            EventMessage<PaymentDTO> eventMessage = objectMapper
                .readValue(message, typeReference);
            ...
            switch (eventMessage.getEventType()) {
                ...
                case UpdatePaymentStatusAsSuccessfulByID:
                    response = updatePaymentStatusAsSuccessful(
                        paymentDTO.getPaymentID(), response);
                    break;
                case UpdatePaymentStatusAsCanceledByID:
                    response = updatePaymentStatusAsCanceled(
                        paymentDTO.getPaymentID(), response);
                    break;
                case UpdatePaymentStatusAsSuccessfulByOrderID:
                    response = updatePaymentStatusByOrderIDAsSuccessful(
                        paymentDTO.getOrderID(), response);
                    break;
                case UpdatePaymentStatusAsCanceledByOrderID:
                    response = updatePaymentStatusByOrderIDAsCanceled(
                        paymentDTO.getOrderID(), response);
                    break;
                case GetPaymentDetailsByOrderID:
                    response = getPaymentDetailsByOrderID(
                        paymentDTO, response);
                    break;
                case UpdatePaymentByOrderIDAsCanceled:
                    response = updatePaymentByOrderIDAsCanceled(
                        paymentDTO, response);
                    break;
                case UpdatePaymentByOrderIDAsSuccesful:
                    response = updatePaymentByOrderIDAsSuccessful(
                        paymentDTO, response);
                    break;
                default:
                    break;
    ...
    private EventMessage<Object> updatePaymentByOrderIDAsSuccessful(PaymentDTO paymentDTO, EventMessage<Object> response){
        String orderID = paymentDTO.getOrderID();
        PaymentResponse paymentResponse = paymentService
            .updatePaymentByOrderIDAsSuccesful(orderID);
        response.setPayload(paymentResponse);
        response.setEventType(EventType.ReturnUpdatePaymentStatus);
        return response;
    } 

    // update payment status
    private EventMessage<Object> updatePaymentByOrderIDAsCanceled(PaymentDTO paymentDTO, EventMessage<Object> response){
        String orderID = paymentDTO.getOrderID();
        PaymentResponse paymentResponse = paymentService
            .updatePaymentByOrderIDAsCanceled(orderID);
        response.setPayload(paymentResponse);
        response.setEventType(EventType.ReturnUpdatePaymentStatus);
        return response;
    } 
    ...
    private EventMessage<Object> getPaymentDetailsByOrderID(PaymentDTO paymentDTO, EventMessage<Object> response){
        String orderID = paymentDTO.getOrderID();
        PaymentDetails paymentDetails = paymentManagementService
            .getPaymentDetails(orderID);
        response.setPayload(paymentDetails);
        response.setEventType(EventType.ReturnPaymentDetails);
        return response;
    }
